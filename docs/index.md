...
> **What this is**: real open issues in the product, codebase, or infrastructure that require code or ops action.
> **Not the same as Audit Findings** — AU-* findings (unverified claims, doc drift, compliance) are tracked separately in [`docs/index-audit-summary.md §Audit Findings`](./index-audit-summary.md).
>
...
| ID | Title | Status | Canonical doc |
|----|-------|--------|---------------|
| TD-001 | No rate limiting on API | open | technical-debt-register |
| TD-007 / GAP-008 | /delete user data not implemented | open | gap-analysis + technical-debt-register |
| TD-005 | Dockerfile uses `prisma db push` in production (confirmed) | open | technical-debt-register |
| GAP-001 / TD-011 | Trigger payday not persisted in Period | open | gap-analysis + technical-debt-register |
| TD-009 / GAP-003 | Notification dedup lost on container restart | open | gap-analysis + technical-debt-register |

### P2 — Product Gaps

| ID | Title | Status | Canonical doc |
|----|-------|--------|---------------|
| GAP-004 / TD-003 | Period rollover timing off by UTC offset | open | gap-analysis |
| GAP-007 | EF contribution silently changes limit on target change | open | gap-analysis |
| GAP-012 | `s2sDaily` naming ambiguity (snapshot vs live) | open | gap-analysis |
| GAP-013 | `emergencyFund.targetAmount` derived not stored | open | gap-analysis |

### Recently Fixed

| ID | Title | Fixed |
|----|-------|-------|
| TD-C002 | CORS open to all origins | FIXED 2026-03-20 |
| TD-C003 | auth_date not validated (replay attack) | FIXED 2026-03-20 |
| TD-C001 | Cron rollover used incomes[0].paydays | FIXED 2026-03-20 |
| TD-008 | s2sActual could be negative in DailySnapshot | FIXED 2026-03-20 |
